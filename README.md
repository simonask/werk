# Werk: Command Runner and Build System
...
- **Declarative:** Advanced build logic is difficult to express. If the limited
  expression support in `werk` is insufficient (and can't be easily supported in
  the model), consider using a more advanced solution like GNU make, CMake,
  ninja-build, scons, cargo-script, etc.
...
```toml
[global]
# Put build artifacts in `<ROOT>/target`
out-dir = "target"
...
```toml
# Configuration of werk itself. These are not variables, but affect how werk operates,
